...
import { dashboardAPI, practiceAPI, queryKeys } from '@/lib/api';
import {
  MessageSquare,
  Send,
  Settings,
...
export default function ScenarioPracticePage() {
  ...
  const startSessionMutation = useMutation({
    mutationFn: practiceAPI.startChatSession,
    onSuccess: (data) => {
      ...
      setMessages([
        {
          ...
          sender: 'ai',
          timestamp: new Date(data.data.initialMessage.timestamp),
        },
      ]);
    },
    onError: (error) => {
  ...
  const sendMessageMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      // Update user message with feedback if available
      if (data.data.userMessage.feedback) {
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === variables.userMessageId
              ? { ...msg, feedback: data.data.userMessage.feedback }
              : msg,
          ),
        );
      }
  ...
  const endSessionMutation = useMutation({
    mutationFn: practiceAPI.endChatSession,
    onSuccess: () => {
      setSessionId(null);
      setMessages([]);
    },
  ...
  // Start chat session when scenario is loaded
  useEffect(() => {
    if (scenario && !sessionId && !startSessionMutation.isPending) {
      startSessionMutation.mutate(scenarioId!);
    }
  }, [scenario, scenarioId, sessionId, startSessionMutation]);
  ...
  const handleSendMessage = async () => {
    ...
    // Use regular API
    try {
      const response = await practiceAPI.sendMessage(sessionId, inputValue);
      ...
      // Update user message with feedback if available
      if (response.data.userMessage.feedback) {
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === userMessage.id
              ? { ...msg, feedback: response.data.userMessage.feedback }
              : msg,
          ),
        );
      }
  ...
  if (scenarioLoading || modeLoading || startSessionMutation.isPending) {
    return (
    ...
    <div className='flex h-[calc(100vh-4rem)] bg-background overflow-hidden'>
      ...
      >
        ...
        <div className='p-4 space-y-6 overflow-y-auto h-[calc(100%-80px)]'>
          ...
          {/* Chat Behavior */}
          <div className='space-y-3'>
            <Label className='text-base font-medium'>Chat Behavior</Label>
            <div className='space-y-4'>
