...
export default function ChatPage() {
    ...
    const createSession = async () => {
      try {
        const response = await fetch(
          '/api/practice/scenarios/clxgy8q2b000008l3f0zihy8k/start',
          {
            method: 'POST',
          },
        );
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch(
        `/api/practice/sessions/${sessionId}/message`,
        {
          method: 'POST',
          headers: {
      ...
      if (data.userMessage.feedback) {
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === userMessage.id
              ? { ...msg, feedback: data.userMessage.feedback }
              : msg,
          ),
        );
      }
